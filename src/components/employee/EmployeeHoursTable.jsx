...
const EmployeeHoursTable = ({ data, rowsPerPage = 5 }) => {
    ...
    return (
        <div className="bg-white shadow-xl rounded-xl p-6 mt-6">
            <h2 className="text-xl font-bold text-[#1742c4] mb-4 pb-2 border-b-2 border-[#1742c4] flex items-center gap-2">
                Employee Hours Table <Clock className="w-5 h-5" />
            </h2>
                ...
                <table className="w-full min-w-[400px] divide-y divide-purple-200">
                    <thead className="bg-[#1742c4]">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Normal Time</th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Overtime Hours</th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Double Time Hours</th>
                        </tr>
                    </thead>
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                        {paginatedData.map((row, index) => (
                            <tr key={index + startIndex} className="hover:bg-purple-50 transition duration-100">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{row.normalTime}</td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{row.overTimeHours}</td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{row.doubleTimeHours}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
