...
export default function PupyGmbHPage({ projects, onStatusToggle, onDelete, onCreateProject }) {
  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-200">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Pupy GmbH</h1>
        ...
        {projects.length > 0 ? (
          ...
          <div className="bg-white border border-dashed border-slate-300 rounded-2xl p-8 sm:p-12 text-center flex flex-col items-center justify-center space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Get started by creating your first project for Pupy GmbH.</p>
            </div>
            <button onClick={onCreateProject} className="w-full sm:w-auto bg-orange-600 hover:bg-orange-700 text-white font-semibold text-xs py-3 sm:py-2.5 px-5 rounded-xl transition-all shadow-md min-h-[44px]">Create Project</button>
          </div>
        )}
      </div>
    </div>
