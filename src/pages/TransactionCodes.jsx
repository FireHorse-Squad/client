...
const COLUMNS = [
    ...
    { id: 'transaction_code', label: 'TRANSACTION CODE', minWidth: 160 },
    { id: 'occupation_name', label: 'OCCUPATION NAME', minWidth: 200 },
];
...
export default function TransactionCodes() {
    ...
    const [formData, setFormData] = useState({ transaction_code: '', occupation_name: '' });
    ...
    const fetchCodes = useCallback(async () => {
        try {
            ...
            setError(err.response?.data?.message || 'Failed to fetch transaction codes');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleOpenModal = (code = null) => {
        if (code) {
            setEditingCode(code);
            setFormData({ transaction_code: code.transaction_code, occupation_name: code.occupation_name || '' });
        } else {
            resetForm();
        }
        setIsModalOpen(true);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingCode) {
                await api.put(`/transactioncodes/${editingCode.id}`, formData);
            } else {
                await api.post('/transactioncodes', formData);
            }
            await fetchCodes();
            setIsModalOpen(false);
            resetForm();
        } catch (err) {
            setError(err.response?.data?.message || 'Failed to save transaction code');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        setError('');
        try {
            await api.delete(`/transactioncodes/${id}`);
            await fetchCodes();
            setDeleteConfirm(null);
        } catch (err) {
            setError(err.response?.data?.message || 'Failed to delete transaction code');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleBulkDeleteConfirm = async () => {
        ...
        try {
            await Promise.all(selectedIds.map((id) => api.delete(`/transactioncodes/${id}`)));
            await fetchCodes();
            setSelectedIds([]);
        } catch (err) {
            setError(err.response?.data?.message || 'Failed to delete selected transaction codes');
        } finally {
    ...
    const filteredData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(c => (c.transaction_code || '').toLowerCase().includes(lower) || (c.occupation_name || '').toLowerCase().includes(lower));
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center border border-slate-200/80 shadow-xs">
                            <svg className="w-5 h-5 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
                        </div>
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Transaction Codes</h1>
                            <p className="text-xs text-slate-500 font-medium">Manage payroll transaction code mappings.</p>
                        </div>
                    </div>
                    <button onClick={() => handleOpenModal()} className="w-full sm:w-auto px-4 py-3 sm:py-2 bg-[#1742c4] hover:bg-blue-800 text-white text-xs font-bold rounded-lg shadow transition flex items-center justify-center gap-2 min-h-[44px]"><Plus size={16} /> Add Code</button>
                </div>
            </div>
            ...
            <div className="w-full bg-white py-4 sm:py-6 px-2 sm:px-4 flex flex-col font-sans">
                <div className="w-full flex flex-col bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div><h1 className="text-xl font-bold text-slate-800">Transaction Codes</h1><p className="text-xs text-slate-500 mt-1">{filteredData.length} codes</p></div>
                        <input type="text" placeholder="Search codes..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setPage(0); }} className="px-3 py-1.5 border border-slate-300 rounded-lg text-xs w-48" />
                    </div>
                    {selectedIds.length > 0 && (
                        <div className="bg-red-50 border-b border-red-200 px-6 py-3 flex items-center justify-between">
                            <span className="text-sm font-semibold text-red-700">
                                {selectedIds.length} code{selectedIds.length > 1 ? "s" : ""} selected
                            </span>
                            <button
                        ...
                        <table className="w-full border-separate border-spacing-0 table-fixed select-text">
                            <thead className="sticky top-0 z-20">
                                <tr>
                                    <th style={{ minWidth: `${COLUMNS[0].minWidth}px`, width: `${COLUMNS[0].minWidth}px`, backgroundColor: HEADER_BG }} className="text-white font-semibold text-xs py-3.5 px-2 uppercase tracking-wider text-center border-r border-indigo-900/40 select-none rounded-tl-sm">
                                        <button onClick={handleSelectAll} className="hover:opacity-80 transition" title={selectedIds.length === paginatedData.length && paginatedData.length > 0 ? "Deselect All" : "Select All Codes"}>
                            ...
                            <tbody className="divide-y divide-slate-200">
                                {paginatedData.length > 0 ? paginatedData.map((row, rowIndex) => {
                                    ...
                                    <tr><td colSpan={COLUMNS.length + 1} className="py-16 text-center text-slate-400 bg-white"><div className="flex flex-col items-center justify-center"><div className="text-slate-300 text-5xl mb-3">📭</div><p className="text-base font-semibold text-slate-500">No transaction codes found</p></div></td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                    <div className="px-6 py-4 bg-white border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4 text-xs sm:text-sm text-[#2D328F] select-none">
                        <div className="text-center md:text-left">Showing <strong className="text-[#2D328F]">{Math.min(filteredData.length, page * rowsPerPage + 1)}</strong> to <strong className="text-[#2D328F]">{Math.min(filteredData.length, (page + 1) * rowsPerPage)}</strong> of <strong className="text-[#2D328F]">{filteredData.length}</strong> codes</div>
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
                        <div className="p-6 text-center"><div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4 text-red-600"><AlertCircle size={32} /></div><h3 className="text-xl font-bold text-slate-800 mb-2">Delete Code?</h3><p className="text-slate-500">This action cannot be undone.</p></div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
                        <div className="p-6 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                            <h3 className="text-xl font-bold text-slate-800">{editingCode ? 'Edit Transaction Code' : 'Add Transaction Code'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div><label className="block text-sm font-medium text-slate-700 mb-1">Transaction Code</label><input type="text" required className="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" value={formData.transaction_code} onChange={e => setFormData({ ...formData, transaction_code: e.target.value })} /></div>
                            <div><label className="block text-sm font-medium text-slate-700 mb-1">Occupation Name</label><input type="text" className="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" value={formData.occupation_name} onChange={e => setFormData({ ...formData, occupation_name: e.target.value })} /></div>
                            <div className="flex gap-3 pt-4 border-t border-slate-100 mt-6">
                                ...
                                <button type="submit" disabled={submitting} className="flex-1 py-2 bg-[#1742c4] text-white font-semibold rounded-lg hover:bg-blue-800 transition-colors shadow-md disabled:opacity-50">{submitting ? 'Saving...' : (editingCode ? 'Save Changes' : 'Create Code')}</button>
                            </div>
                        </form>
                    </div>
                </div>
